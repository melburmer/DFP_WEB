    ...
    for data_doc in data_set:
        if data_doc['bin_file_hash'] in selected_data_hash_list:
            ...
            for cur_spec_idx, cur_spec_ch in enumerate(spec_chs_red):
                ...
                fig = go.Figure(data=go.Heatmap(x = t, y = f, z = Sxx_log, colorscale ='Jet'))
                fig.update_xaxes(title_text="Time [sec]")
                fig.update_yaxes(title_text="Frequency [Hz]")
                fig.update_layout(title_text='Spectrogram ch: ' + str(cur_spec_ch+start_channel))
                path_to_save = os.path.join(img_save_path, raw_data_file_name+f"_ch{cur_spec_ch+start_channel}_spec.html")
