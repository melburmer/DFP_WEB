...
def visualize_roc_curves(analysis_result_path, models_to_run, acts_to_run, test_set_models_result_path):
    ...
    try:
        ...
        raise FileNotFoundError(f"Path:{analysis_result_path} cannot be read ")
    ...
    for model_idx, model_id in enumerate(models_to_run):
        ...
        if model_id == 0:
            ...
            raise AssertionError("Error in visualize.py file. Error: Category num should be 5 or 14")
        ...
        all_models_summary[str(model_id)] = [all_act_max_count_nums, all_act_ground_truths, all_act_prob_threshes]
    ...
    for act_idx in range(len(acts_to_run)):  # create figure for each act
        ...
        for prob_idx in range(n_different_prob):
            ...
            for model_idx in range(len(models_to_run)):
                model_id = str(models_to_run[model_idx])
                ...
                fig.add_trace(go.Scatter(x=cur_prob_fa_rates, y=cur_prob_counts_sorted_unique,
                name=label_pretext + ", fa rate-act count", mode="markers"), secondary_y=True)
