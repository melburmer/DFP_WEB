...
class Records_Container(djongo_model.Model):
...
class Testset(djongo_model.Model):
    ACT_CHOICES =   [('digging','digging'),
                     ('excavator','excavator'),
                     ('human','human')]

    test_set_name = models.CharField(max_length=50)
    models_to_run = models.CharField(max_length=100)
    acts_to_run = MultiSelectField(choices=ACT_CHOICES, max_choices=3)
    data_set = djongo_model.ArrayField(model_container=Records_Container, blank=True)
